...
def apply_template_variables(text: str, row: MatrixRow) -> str:
    """
    Replace template variables with actual values from matrix row.
    ...
    replacements = {
        ...
        "{{progress}}": str(row.progress_pct),
        "{{project_url}}": row.url,
        "{{current_state}}": row.current_state,
        "{{progress_pct}}": str(row.progress_pct),
        "{{issue_count}}": str(row.issue_count),
        "{{lead}}": row.lead or "Not assigned",
        "{{health}}": row.health,
        "{{target_date}}": row.target_date or "Not set",
